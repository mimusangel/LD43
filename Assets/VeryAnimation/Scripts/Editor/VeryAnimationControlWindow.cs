                ...
                if (gameObject != null)
                {
                    ...
                    if (boneIndex >= 0)
                    {
                        ...
                        if (va.mirrorBoneIndexes[boneIndex] >= 0)
                        {
                            ...
                            Rect r = args.rowRect;
                            float Margin = (r.height - mirrorTex.height) / 2f;
                            r.height -= Margin * 2f;
                            r.y += Margin;
                            r.width = mirrorTex.width;
                            r.x = args.rowRect.width - (currentX + r.width);
                            if (GUI.Button(r, new GUIContent("", string.Format("Mirror: '{0}'", va.bones[va.mirrorBoneIndexes[boneIndex]].name)), vaw.guiStyleMirrorButton))
                            {
