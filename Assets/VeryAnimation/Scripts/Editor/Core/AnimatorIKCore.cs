    ...
    public class AnimatorIKCore
    {
        ...
        public void HandleGUI()
        {
            ...
            if (ikActiveTarget != IKTarget.None && ikData[(int)ikActiveTarget].enable)
            {
                ...
                var worldPosition = GetObjectSpaceWorldPosition(activeData);
                var hiA = GetStartHumanoidIndex(ikActiveTarget);
                {
                        ...
                        !activeData.autoRotation && va.lastTool != Tool.Move)
                    {
                        #region Rotate
                        EditorGUI.BeginChangeCheck();
                        var rotation = Handles.RotationHandle(Tools.pivotRotation == PivotRotation.Local ? activeData.worldRotation : Tools.handleRotation, worldPosition);
                        if (EditorGUI.EndChangeCheck())
                        {
                    ...
                    else
                    {
                        ...
                        EditorGUI.BeginChangeCheck();
                        var position = Handles.PositionHandle(worldPosition, Tools.pivotRotation == PivotRotation.Local ? activeData.rotation : Quaternion.identity);
                        if (EditorGUI.EndChangeCheck())
                        {
                            ...
                            if (EditorGUI.EndChangeCheck())
                            {
                                ...
                                if (beforeVec.sqrMagnitude > 0f && afterVec.sqrMagnitude > 0f)
                                {
                                    ...
                                    Quaternion rotationY = Quaternion.identity;
                                    {
                                        ...
                                        var beforeVecY = beforeP - toesPos;
                                        var afterVecY = afterP - toesPos;
                                        if (beforeVecY.sqrMagnitude > 0f && afterVecY.sqrMagnitude > 0f)
                                        {
                                            beforeVecY.Normalize();
                                            afterVecY.Normalize();
                                            var lockAxis = Vector3.Cross(beforeVecY, afterVecY);
                                            var deg = Mathf.Atan2(lockAxis.magnitude, Vector3.Dot(beforeVecY, afterVecY)) * Mathf.Rad2Deg;
                                            if (Vector3.Dot(lockAxis, normal) < 0f)
                                                deg = -deg;
                                            rotationY = Quaternion.AngleAxis(deg, normal);
                                        }
                                    }
                                    Quaternion rotationX = Quaternion.identity;
                                    {
                                        ...
                                        if (beforeVecY.sqrMagnitude > 0f && afterVecY.sqrMagnitude > 0f)
                                        {
                                            ...
                                            if (Vector3.Dot(lockAxis, normal) < 0f)
                                                deg = -deg;
                                            rotationX = Quaternion.AngleAxis(deg, normal);
                                        }
                                    }
        ...
        public void TargetGUI()
        {
            ...
            for (int target = 0; target < (int)IKTarget.Total; target++)
            {
                ...
                var position = GetObjectSpaceWorldPosition(ikData[target]);
        ...
        public Vector3 GetObjectSpaceWorldPosition(AnimatorIKData data)
        {
            if (va.dummyObject != null && data.spaceType == AnimatorIKData.SpaceType.Parent)
            {
                var worldPosition = vaw.gameObject.transform.worldToLocalMatrix.MultiplyPoint3x4(data.worldPosition);
                worldPosition = va.dummyObject.gameObjectTransform.localToWorldMatrix.MultiplyPoint(worldPosition);
        ...
        public void SetObjectSpaceWorldPosition(AnimatorIKData data, Vector3 worldPosition)
        {
            if (va.dummyObject != null && data.spaceType == AnimatorIKData.SpaceType.Parent)
            {
                worldPosition = va.dummyObject.gameObjectTransform.worldToLocalMatrix.MultiplyPoint(worldPosition);
                worldPosition = vaw.gameObject.transform.localToWorldMatrix.MultiplyPoint3x4(worldPosition);
            }
            data.worldPosition = worldPosition;
        }
        ...
        public void AnimatorOnAnimatorIK(int layerIndex)
        {
...

            {
                ...
                if (data.enable)
                {
                    animator.SetLookAtPosition(ikSaveWorldToLocalMatrix.MultiplyPoint3x4(GetObjectSpaceWorldPosition(data)));
            ...
            }
            {
                ...
                if (data.enable)
                {
                    animator.SetIKPosition(AvatarIKGoal.LeftHand, ikSaveWorldToLocalMatrix.MultiplyPoint3x4(GetObjectSpaceWorldPosition(data)));
            ...
            }
            {
                ...
                if (data.enable)
                {
                    animator.SetIKPosition(AvatarIKGoal.RightHand, ikSaveWorldToLocalMatrix.MultiplyPoint3x4(GetObjectSpaceWorldPosition(data)));
            ...
            }
            {
                ...
                if (data.enable)
                {
                    animator.SetIKPosition(AvatarIKGoal.LeftFoot, ikSaveWorldToLocalMatrix.MultiplyPoint3x4(GetObjectSpaceWorldPosition(data)));
            ...
            }
            {
                ...
                if (data.enable)
                {
                    animator.SetIKPosition(AvatarIKGoal.RightFoot, ikSaveWorldToLocalMatrix.MultiplyPoint3x4(GetObjectSpaceWorldPosition(data)));
        ...
        private void ChangeSpaceType(IKTarget target, AnimatorIKData.SpaceType spaceType)
        {
            ...
            var position = GetObjectSpaceWorldPosition(data);
            var rotation = data.worldRotation;
            data.spaceType = spaceType;
            SetObjectSpaceWorldPosition(data, position);
