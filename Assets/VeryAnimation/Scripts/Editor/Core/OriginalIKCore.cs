...
namespace VeryAnimation
{
    ...
    public class OriginalIKCore
    {
        ...
        public class OriginalIKData
        {
            ...
            public GameObject tip { get { return joints != null && joints.Count > 0 ? joints[0].bone : null; } }
            public GameObject root { get { return joints != null && joints.Count > 0 ? joints[level - 1].bone : null; } }
            public GameObject turn(int l) { return joints != null && level > l ? joints[l].bone : null; }
        ...
        public class SolverCCD : SolverBase
        {
            ...
            public override void Update(Vector3 targetPos, Quaternion? targetRotation)
            {
                ...
                for (int i = 0; i < Iteration; i++)
                {
                    for (int j = 1; j < boneTransforms.Length; j++)
                    {
                        ...
                        if (localEffectorPos.sqrMagnitude <= 0f || localTargetPos.sqrMagnitude <= 0f)
                            continue;
                        {
                            var rotationAdd = Quaternion.FromToRotation(localEffectorPos.normalized, localTargetPos.normalized);
                            if (weights[j] != 1f)
                ...
                {
                    var axis = tip.position - root.position;
                    if (axis.sqrMagnitude > 0f)
                    {
                        axis = root.transform.worldToLocalMatrix.MultiplyVector(axis.normalized);
            ...
            public override float GetSwivel()
            {
                ...
                {
                    Vector3 lockAxis = Vector3.Cross(vecBefore, vecAfter);
                    if (lockAxis.sqrMagnitude > 0f)
                    {
                        var dot = Vector3.Dot(vecBefore, vecAfter);
                        var theta = Mathf.Atan2(lockAxis.magnitude, dot);
                        result = theta * Mathf.Rad2Deg;
                        var dot2 = Vector3.Dot(lockAxis, root.position - tip.position);
                        if (dot2 > 0f)
                            result = -result;
                ...
                {
                    {
                        ...
                        linearizationForward = Quaternion.Inverse(Quaternion.LookRotation(vLower));
                    }
                    {
                        var invRot = Quaternion.Inverse(lower.rotation);
                        linearizationLower = Quaternion.FromToRotation(invRot * (tip.position - lower.position).normalized, invRot * (lower.position - upper.position).normalized);
                    }
                    {
                        lowerAxis = Quaternion.LookRotation((lower.position - upper.position).normalized) * Vector3.up;
        ...
        public int CreateIKData(GameObject jointTip)
        {
            {
        ...
        public bool ChangeSolverType(int target, SolverType solverType)
        {
            ...
            if (ikData[target].solver.isValid)
            {
                ...
                case SolverType.LimbIK:
                    #region Auto set limbDirection
                    {
                        ...
                        solverLimb.swivel = ikData[target].swivel = 0f;
                    }
                    #endregion
                    break;
                }
        ...
        public bool ChangeTypeSetting(int target, float add)
        {
            ...
            case SolverType.LookAt:
                if (add > 0)
                {
                    var t = ikData[target].root.transform.parent;
                    for (int i = 0; i < add; i++)
                    {
                        if (t == null) break;
                        if (!IsValidAddBone(t.gameObject))
        ...
        private void UpdateReorderableList()
        {
            ...
            ikReorderableList.onCanAddCallback = (ReorderableList list) =>
            {
                ...
                return ikData.FindIndex((data) => data.tip == va.bones[va.selectionActiveBone]) < 0;
            };
#endif
        ...
        private bool IsErrorJoint(int target, int index)
        {
            ...
            if (ikData[target].solver == null || !ikData[target].solver.isValid)
            {
                if (index < 0 || index >= ikData[target].level)
                    return true;
                if (ikData[target].joints[index].bone == null)
                    return true;
                if (EditorCommon.ArrayIndexOf(va.bones, ikData[target].joints[index].bone) < 0)
                    return true;
                {
                    int count = 0;
                    int level = 0;
                    ...
                    while (t != null)
                    {
                        if (ikData[target].joints[level].bone != null && t == ikData[target].joints[level].bone.transform)
                        {
                            count++;
                            level++;
                        }
                        if (t.gameObject == ikData[target].root)
                            break;
                        if (t.gameObject == ikData[target].joints[index].bone)
                            break;
                        if (EditorCommon.ArrayIndexOf(va.bones, t.gameObject) < 0)
                            break;
                        t = t.parent;
                    }
                    if (count != index + 1)
        ...
        private int GetMirrorTarget(int target)
        {
            if (target >= 0 && target < ikData.Count)
            {
                var boneIndex = va.BonesIndexOf(ikData[target].tip);
                if (boneIndex >= 0 && va.mirrorBoneIndexes[boneIndex] >= 0)
                {
                    for (int i = 0; i < ikData.Count; i++)
                    {
                        if (ikData[i].tip == va.bones[va.mirrorBoneIndexes[boneIndex]])
                            return i;
                    }
                }
            }
        ...
        public void SynchroSet(int target)
        {
            ...
            case OriginalIKData.SpaceType.Local:
                if (data.root != null && data.root.transform.parent != null)
                {
                    data.position = data.root.transform.parent.worldToLocalMatrix.MultiplyPoint3x4(t.position);
                    data.rotation = Quaternion.Inverse(data.root.transform.parent.rotation) * t.rotation;
                }
                else
                {
                    data.position = t.position;
        ...
        public void UpdateIK(float time)
        {
            ...
            va.transformPoseSave.ResetOriginalTransform();
        ...
        public void HandleGUI()
        {
...

            {
                ...
                else
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    var position = Handles.PositionHandle(activeData.worldPosition, Tools.pivotRotation == PivotRotation.Local ? activeData.worldRotation : Quaternion.identity);
                    if (EditorGUI.EndChangeCheck())
                    {
            ...
            for (int target = 0; target < ikData.Count; target++)
            {
                    ...
                    EditorCommon.ArrayContains(ikTargetSelect, target))
                {
                    #region Active
                    {
                        ...
                        {
                            Handles.color = Color.white;
                            var posA = va.editBones[ikData[target].solver.boneIndexes[ikData[target].solver.boneIndexes.Length - 1]].transform.position;
                            Handles.DrawLine(position, posA);
                        }
                        Handles.color = vaw.editorSettings.settingIKTargetActiveColor;
                        if (ikData[target].solverType == SolverType.LookAt)
                            ...
                            Handles.CubeHandleCap(0, position, rotation, HandleUtility.GetHandleSize(position) * vaw.editorSettings.settingIKTargetSize, EventType.Repaint);
                    }
                    #endregion
                }
                else
                {
                    ...
                    Handles.FreeMoveHandle(position, rotation, HandleUtility.GetHandleSize(position) * vaw.editorSettings.settingIKTargetSize, uSnapSettings.move, (id, pos, rot, size, eventType) =>
                    {
                        ...
                        if (ikData[target].solverType == SolverType.LookAt)
                            ...
                            Handles.CubeHandleCap(id, position, rotation, HandleUtility.GetHandleSize(position) * vaw.editorSettings.settingIKTargetSize, eventType);
                    });
            ...
            case SolverType.LookAt:
                {
                    ...
                    Vector3 vec = data.position - t.position;
                    var normal = t.rotation * Vector3.right;
                    var dot = Vector3.Dot(vec, normal);
                    data.position -= normal * dot;
                }
                break;
            default:
                {
                    var boneIndex = va.BonesIndexOf(data.tip);
                    data.rotation = data.tip.transform.parent.rotation * va.boneSaveTransforms[boneIndex].localRotation;
                }
                break;
            }
            va.SetUpdateIKtargetOriginalIK(target);
